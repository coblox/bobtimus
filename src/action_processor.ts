...
export class ActionSelector {
  public static select(entity: Entity): Result<Action, Error> {
    if (entity.class && entity.class.some((e: string) => e === "swap")) {
      const swap = entity as Swap;

      return ActionSelector.selectAction(swap);
    }
    return Result.err(new Error("Internal Error: Not a swap action "));
  }

  private static selectAction(swap: Swap): Result<Action, Error> {
